...
**Versão:** 1.6.3 | **Licença:** MIT | **Python:** >= 3.10
...
- **Não faz streaming contínuo** — a versão 1.6.3 suporta Autoloader em `available_now`, que é execução finita com checkpoint; processamento contínuo fica fora do escopo.
...
pip install build
python -m build
# → dist/lakehouse_ingestion_framework-1.6.3-py3-none-any.whl

# Upload para UC Volume
databricks fs cp dist/lakehouse_ingestion_framework-1.6.3-py3-none-any.whl \
  dbfs:/Volumes/<catalog>/<schema>/libs/

# No notebook Databricks:
%pip install /Volumes/<catalog>/<schema>/libs/lakehouse_ingestion_framework-1.6.3-py3-none-any.whl
...
Para streaming contínuo, não. A versão 1.6.3 suporta Autoloader em `available_now`, que é uma execução finita com checkpoint e `foreachBatch`. Para processamento contínuo, considere Delta Live Tables (DLT) ou Structured Streaming direto.
