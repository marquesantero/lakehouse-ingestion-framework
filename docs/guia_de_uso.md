...
pip install build
python -m build
# gera: dist/contractforge-2.5.1-py3-none-any.whl
```
...
```bash
# via Databricks CLI
databricks fs cp dist/contractforge-2.5.1-py3-none-any.whl \
...
2. Source: **Volume**
3. File path: `/Volumes/<catalog>/<schema>/libs/contractforge-2.5.1-py3-none-any.whl`
4. Install
...
print(contractforge.__version__)  # 2.5.1
...
```python
%pip install /Volumes/<catalog>/<schema>/libs/contractforge-2.5.1-py3-none-any.whl
```

Se o cluster não permite `%pip` por restrição:

```python
%pip install --index-url https://<seu_pypi_privado> contractforge==2.5.1
```
